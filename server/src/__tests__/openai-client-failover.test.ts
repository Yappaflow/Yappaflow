...
describe("createChatCompletion — capacity-aware provider routing", () => {
  ...
  it("routes a 32k request to OpenRouter first, skipping DeepSeek's 8k cap", async () => {
    ...
    // `qwen/qwen3-coder` on OpenRouter carries 262k input context and
    // — unlike the native DeepSeek API — doesn't enforce the 8k
    // output cap, so it can handle a 32k Shopify generation. It's also
    // a purpose-built code model, so the visual quality of generated
    // themes is materially better than DeepSeek V3.2's output.
    expect(createSpy.mock.calls[0][0].model).toBe("qwen/qwen3-coder");
...
describe("createChatCompletion — per-provider model normalization", () => {
  ...
  it("swaps DeepSeek-native name for OpenRouter's large-context model on big requests", async () => {
    ...
    // Caller asks for 'deepseek-chat' (DeepSeek bare name). On large
    // outputs OpenRouter must swap to its large-context code model
    // (`qwen/qwen3-coder` — 262k context, dedicated code model with
    // better Liquid/HTML design quality than DeepSeek V3.2), not the
    // small analysis default whose upstream endpoint caps at 32k total
    // context and breaks on big generations.
    const { createChatCompletion } = await loadClient();
 ...
 * have 128k+ input context AND no hidden output cap — currently
 * `qwen/qwen3-coder` (Alibaba's 480B-A35B MoE code model, 262k
 * context, ~$0.22/$1.00 per 1M, dedicated code training that beats
 * DeepSeek V3.2 on Liquid/HTML design quality).
 */
describe("createChatCompletion — large-output routing on OpenRouter", () => {
  ...
  it("caller-supplied OpenRouter model survives even on a big request", async () => {
    ...
    // quirks). Qwen 72B here stands in for "any non-default choice";
    // the provider's own largeOutputModel (qwen/qwen3-coder via
    // OpenRouter) would be vacuous to test since the upgrade branch
    // would pick it anyway.
    const { createChatCompletion } = await loadClient({ deepseekApiKey: "" });
    script = [{ ok: "ok" }];
  ...
  it("DeepSeek-native model on a big request → swapped to the large-context model on OpenRouter", async () => {
    ...
    // should land on the large-context model (qwen/qwen3-coder via
    // OpenRouter) — not the ambiguous-name-error path.
    const { createChatCompletion } = await loadClient();
